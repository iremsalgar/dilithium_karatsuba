...
int crypto_sign_keypair(uint8_t *pk, uint8_t *sk) {
  ...
  s1hat = s1;
  polyvec_matrix_pointwise_toomcook(&t1, mat, &s1hat);
  polyveck_reduce(&t1);
...
rej:
  ...
  z = y;
  polyvec_matrix_pointwise_toomcook(&w1, mat, &z);
  polyveck_reduce(&w1);
  ...
  for (unsigned int i = 0; i < L; ++i) {
    poly tmp;
    poly_mul_toomcook(&tmp, &cp, &s1.vec[i]);
    z.vec[i] = tmp;
  }
  ...
  for (unsigned int i = 0; i < K; ++i)
	poly_mul_toomcook(&h.vec[i], &cp, &s2.vec[i]);
  ...
  for (unsigned int i = 0; i < K; ++i)
	poly_mul_toomcook(&h.vec[i], &cp, &t0.vec[i]);
                       ...
                       const uint8_t *pk)
{
  ...
  for (unsigned int i = 0; i < K; ++i) {
      poly_mul_toomcook(&t1_mul.vec[i], &cp, &t1.vec[i]);
  }
